...
def genbankDownloadRecords(idList, destDir, batchSize, delay = 30,
    ...
    for i in range(0, len(newIdList), batchSize) :
        ...
        _genbankDownloadBatch(batch, destDir)
        time.sleep(delay)

### ** _genbankDownloadBatch(idBatch, destDir)

def _genbankDownloadBatch(idBatch, destDir) :
...
### ** _genbankSummarizeRecord(fi, outfmt, hashConstructor, gb_record_fmtdict, gb_cds_fmtdict)
...
def _main_extract_CDS(args = None, stdout = sys.stdout, stderr = sys.stderr,
    ...
    for fi in args.genbank_records :
        record = SeqIO.read(fi, "genbank")
        if not args.actionFlags.get("DoCount", False) :
            (summaryString, uniqueSeq, newSeq) = (
                _genbankSummarizeRecord(record, args.outfmt, args.hash, uniqueSeq))
            stdout.write(summaryString)
        else :
            count = len([x for x in record.features if x.type == "CDS"])
            stdout.write(record.annotations["gi"] + "\t" + str(count) + "\n")
...
def _makeParser_extract_CDS() :
    ...
    parser.add_argument("-o", "--outfmt", metavar = "FORMAT",
                        ...
                        help = "String describing the output columns for summaries")
    # --count
